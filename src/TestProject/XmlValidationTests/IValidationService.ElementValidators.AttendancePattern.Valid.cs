...
namespace TestProject.XmlValidationTests
{
        ...
        where T : XCRI.Validation.IValidationService<String>
    {
        ...
        protected XCRI.Validation.ContentValidation.ElementValidator GetElementValidator_AttendancePattern()
        {
            var elementValidator = new XCRI.Validation.ContentValidation.ElementValidator()
            {
                NamespaceManager = this.GetNamespaceManager(),
                XPathSelector = "//xcri12:attendancePattern"
            };
            return elementValidator;
        }

        [TestMethod]
        public void Valid_AttendancePattern_WithIdentifier()
        {
            var elementValidator = this.GetElementValidator_AttendancePattern();
            ...
            var vr = elementValidator
                .Validate(System.Xml.Linq.XDocument.Parse(Resources.IValidationService.Valid.ElementValidation.AttendancePattern.WithIdentifier).Root)
