...
namespace TestProject.XmlValidationTests
{
        ...
        where T : XCRI.Validation.IValidationService<String>
    {
        ...
        protected XCRI.Validation.ContentValidation.ElementValidator GetElementValidator_StudyMode()
        {
            var elementValidator = new XCRI.Validation.ContentValidation.ElementValidator()
            {
                NamespaceManager = this.GetNamespaceManager(),
                XPathSelector = "//xcri12:studyMode"
            };
            return elementValidator;
        }

        [TestMethod]
        public void Valid_StudyMode_WithIdentifier()
        {
            ...
            var vr = elementValidator
                .Validate(System.Xml.Linq.XDocument.Parse(Resources.IValidationService.Valid.ElementValidation.StudyMode.WithIdentifier).Root)
